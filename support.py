...
def connect_db():
    ...
    cur.execute(
        ...
        month INTEGER NOT NULL CHECK (month >= 1 AND month <= 12), amount INTEGER NOT NULL, expense VARCHAR(10) NOT NULL,
...
def get_current_month_budget(user_id):
    """
    Retrieves the user's budget for the current month and year.
    Args:
        user_id: The ID of the user.
    Returns:
        The user's budget as a float, or None if not found.
    """
    # Get current date
    ...
    query = """SELECT amount FROM user_budgets WHERE user_id = {} AND month = {} AND year = {}""".format(
        user_id, current_month, current_year)
    result = execute_query('search', query)

    if result:
        return result[0][0]  # Return the budget amount
    else:
        return None  # Budget not found for this month/year
    
def get_current_month_expenses(user_id):
  """
  Fetches the sum of current month's expense values for each category for a specific user.

  Args:
      user_id: The ID of the user to get expenses for (integer).

  Returns:
      A dictionary containing the sum of expenses for each category ('Spends', 'Earning', 'Saving', 'Investment') or None if no data found.
  """
  connection, cursor = connect_db()
  today = datetime.date.today()
  current_month = today.month
  print(current_month)
  query = f"""
  SELECT expense, SUM(amount) AS total_amount
  FROM user_expenses
  WHERE user_id = {user_id} AND strftime('%m', pdate) = '04'
  GROUP BY expense;
  """
  print(query)
  try:
    ...
    print(data)
    if not data:
      return None  # No data found for current month
    expenses = {}
    for row in data:
      expenses[row[0]] = row[1]  # Category as key, sum(amount) as value
    return expenses
...
def get_remaining_balance(user_id):
    """
  Fetches the remaining_balance current month's expense values for Spend category for a specific user.
  Args:
      user_id: The ID of the user to get expenses for (integer).
  Returns:
      A dictionary containing integer value the remaining_balance or None if no data found.
  """
  
    connection, cursor = connect_db()
    # Retrieve budget for current month/year
    budget = get_current_month_budget(user_id)

        # Retrieve previous expenses for current month/year
    current_expense= get_current_month_expenses(user_id)
    previous_amount= current_expense['Spend']
    try:
        if budget is None:
            return 0
        else:
            return budget - previous_amount
    except sqlite3.Error as e:
        print("Error fetching expenses:", e)
        return None
    finally:
